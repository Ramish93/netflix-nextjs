...
function Row({ title, movies }: Props) {
  ...
  return (
    <div className="h-40 space-y-0.5 md:space-y-2">
      <h2 className="w-56 cursor-pointer text-sm font-semibold text-[#e5e5e5] transition duration-200 hover:text-[#a41e1e] md:text-2xl">
        {title}
      </h2>
      <div className="group relative md:-ml-2">
        ...
        <div
          ref={rowRef}
          className="flex item-center space-x-0.5 scrollbar-hide overflow-x-scroll md:space-x-2.5 md:p-2"
        >
          {movies.map((movie) => (
